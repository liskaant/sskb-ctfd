    ...
    function getSolves(id) {
      return CTFd.api.get_challenge_solves({ challengeId: id }).then(response => {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if(data[i].award) {
              let text = '<b>' + ordinalize(data[i].award.solve_num) + '</b>';
              if (first_blood_bonus[data[i].award.solve_num - 1])
                  text += ' (+' + first_blood_bonus[data[i].award.solve_num - 1] + ')';
              if (data[i].award.fb_category)
                   text += ' (' + data[i].award.fb_category + ')';
              if (i < 3)
                  text = '<span class="award-icon award-medal-' + ordinalize(data[i].award.solve_num) + '"></span>' + text;
              else
