...
from .util import filter_standings

scoreboard = Namespace("scoreboard", __name__)

@scoreboard.route("/top/<int:count>/<category>")
@scoreboard.param("count", "How many top teams to return")
@scoreboard.param("category", "Filter only users in specific category")
class ScoreboardDetail(Resource):
	@check_account_visibility
	@check_score_visibility
	@cache.cached(timeout=60)
	def get(self, count: int, category: str):
		response = {}

		standings = filter_standings(get_standings(count=count), category)
...
class ScoreboardList(Resource):
	...
	def get(self, category: str):
		standings = filter_standings(get_standings(), category)
		response = []
